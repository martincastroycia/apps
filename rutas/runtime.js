...
function pfbarra(hi){
  if(!hi || !hi.h) return '';
  var hoy=new Date(), m0=hoy.getMonth(), h='<div class="pfmes">';
  for(var i=0;i<hi.h.length;i++){
    var mi=(m0-(11-i)+24)%12;
    h+='<span class="'+(hi.h.charAt(i)==='1'?'pfmy':'pfmn')+'">'+MESC[mi]+'</span>';
  }
  return h+'</div>';
}
function pfniv(c, T){
  ...
  if(hi){
    h+='<div class="pfnt">C\u00f3mo viene</div><div class="pfhi">'
      ...
      + '</div>' + pfbarra(hi);
    if(hi.lt){
      var tot=0; hi.lt.forEach(function(v){ tot+=v; });
      if(tot>0) h+='<div class="pfhi">Litros de los \u00faltimos meses: '
        + hi.lt.map(function(v){ return v? (Math.round(v*10)/10).toLocaleString('es-AR') : '\u00b7'; }).join(' \u00b7 ')
        + ' \u00b7 <b>'+(Math.round(tot*10)/10).toLocaleString('es-AR')+' L</b> en total</div>';
    }
  }
